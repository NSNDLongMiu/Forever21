...
let {registeradmin} = require("./userServices");
let {registeradminValidator} = require("./userValidator");

let {isLogging} = require("./userServices");
let {signIn} = require("./userServices");
let {loginValidator} = require("./userValidator");

router.post("/admindetail", async (req, res, next) => {
  try {
    let validator = await registeradminValidator(req);
    if (validator !== null) {
      return res.send({success: "2" , message: validator});
    } 
    else {
      let registed = await registeradmin(req.body);
      if (registed == true) {
